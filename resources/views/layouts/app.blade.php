...
<head>
    <meta charset="UTF-8">
    <title>{{__(setting('app_name'))}} | {{__(setting('app_short_description'))}}</title>
        ...
        <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right">
                    ...
                    @else
                        <a href="/login" class="dropdown-item"> <i class="fas fa-user mr-2"></i> {{__('auth.login')}}</a>
                    @endif
                </div>
            </li>
        </ul>
...
<!-- Content Wrapper. Contains page content -->
    ...
    <footer class="main-footer border-0 shadow-sm">
        <div class="float-sm-right d-none d-sm-block">
            <b>{{__('lang.version')}}</b> {{implode('.',str_split(substr(config('installer.currentVersion','v100'),1,3)))}}
        </div>
        <strong>{{__('lang.copyright')}} {{date('Y')}} <a href="{{url('/')}}">{{setting('app_name')}}</a>. </strong>{{__('lang.all_rights_reserved')}}
    </footer>
